...
const (
	...
	LoadbalancerWeightAnnotation = "dockermachine.infrastructure.cluster.x-k8s.io/weight"
)
...
const (
	// DevMachineDockerCGroupReadyCondition documents the status of cgroups inside the container
	// generated by a DevMachine's docker backend.
	DevMachineDockerCGroupReadyCondition string = "CGroupReady"

	// DevMachineDockerCGroupReadyWaitingForContainerReason documents when the system is waiting for the creation of
	// the container for a DevMachine's docker backend.
	DevMachineDockerCGroupReadyWaitingForContainerReason = "WaitingForContainer"

	// DevMachineDockerCGroupReadyReason documents when cgroups inside the container for a DevMachine's docker backend
	// are ready.
	DevMachineDockerCGroupReadyReason = clusterv1.ReadyReason

	// DevMachineDockerCGroupReadyReason documents when cgroups inside the container for a DevMachine's docker backend
	// are not ready.
	DevMachineDockerCGroupNotReadyReason = clusterv1.NotReadyReason

	// DevMachineDockerCGroupReadyInternalErrorReason documents an unexpected error happened when starting
	// cgroups inside the container for a DevMachine's docker backend are not ready.
	DevMachineDockerCGroupReadyInternalErrorReason = clusterv1.InternalErrorReason
)
...
const (
	...
	// DevMachineDockerPreLoadedImagesReadyWaitingForCGroupReason documents when the system is waiting for
	// cgroups inside the container for a DevMachine's docker backend.
	DevMachineDockerPreLoadedImagesReadyWaitingForCGroupReason = "WaitingForCGroup"
	...
	// preloading images inside the container for a DevMachine's docker backend are not ready.
	DevMachineDockerPreLoadedImagesReadyInternalErrorReason = clusterv1.InternalErrorReason
)

// CloudInitOrIgnitionCompleted condition and corresponding reasons for a DevMachine's docker backend.
const (
	// DevMachineDockerCloudInitOrIgnitionCompletedCondition documents the progress of the cloud-init / ignition
	// script executed after the provisioning of the container generated by a DevMachine's docker backend.
	DevMachineDockerCloudInitOrIgnitionCompletedCondition string = "CloudInitOrIgnitionCompleted"

	// DevMachineDockerCloudInitOrIgnitionCompletedWaitingForContainerReason documents when the system is waiting for the creation of
	// the container for a DevMachine's docker backend.
	DevMachineDockerCloudInitOrIgnitionCompletedWaitingForContainerReason = "WaitingForContainer"

	// DevMachineDockerCloudInitOrIgnitionCompletedWaitingForCGroupReason documents when the system is waiting for
	// cgroups inside the container for a DevMachine's docker backend.
	DevMachineDockerCloudInitOrIgnitionCompletedWaitingForCGroupReason = "WaitingForCGroup"

	// DevMachineDockerCloudInitOrIgnitionCompletedWaitingForPreloadedImagesReason documents when the system is waiting for
	// preloaded images inside the container for a DevMachine's docker backend.
	DevMachineDockerCloudInitOrIgnitionCompletedWaitingForPreloadedImagesReason = "WaitingForPreloadedImages"

	// DevMachineDockerPreLoadedImagesReadyReason documents when preloaded the cloud-init / ignition
	// script executed inside the container for a DevMachine's docker backend is completed.
	DevMachineDockerCloudInitOrIgnitionCompletedReason = "Completed"

	// DevMachineDockerPreLoadedImagesReadyReason documents when preloaded the cloud-init / ignition
	// script executed inside the container for a DevMachine's docker backend is not completed.
	DevMachineDockerCloudInitOrIgnitionNotCompletedReason = "NotCompleted"

	// DevMachineDockerCloudInitOrIgnitionFailedReason documents when preloaded the cloud-init / ignition
	// script executed inside the container for a DevMachine's docker backend failed.
	DevMachineDockerCloudInitOrIgnitionFailedReason = "Failed"

	// DevMachineDockerCloudInitOrIgnitionCompletedInternalErrorReason documents an unexpected error happened when
	// executing the cloud-init / ignition script inside the container for a DevMachine's docker backend are not ready.
	DevMachineDockerCloudInitOrIgnitionCompletedInternalErrorReason = clusterv1.InternalErrorReason
)
...
type DevMachineStatus struct {
	...
	// - Ready, ContainerProvisioned, CGroupReady, PreLoadedImagesReady, CloudInitOrIgnitionCompleted and Paused when using the docker backend.
	// +optional
	// +listType=map
	// +listMapKey=type
