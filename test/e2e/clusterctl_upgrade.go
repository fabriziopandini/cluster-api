...
func ClusterctlUpgradeSpec(ctx context.Context, inputGetter func() ClusterctlUpgradeSpecInput) {
	...
	It("Should create a management cluster and then upgrade all the providers", func() {
		...
		if input.E2EConfig.HasVariable("INIT_WITH_BINARY") {
			binary = input.E2EConfig.GetVariable("INIT_WITH_BINARY")
		}
		...
		// However, given that this cluster could be created with an older version of the API,
		// we are not using ApplyClusterTemplateAndWait because those helpers are linked to last version of the API.
		// instead are are applying the template and waiting using the older version of MachineList (that should work
		// both with older version of the API and the current one).
