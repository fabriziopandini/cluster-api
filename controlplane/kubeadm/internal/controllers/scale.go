...
func (r *KubeadmControlPlaneReconciler) preflightChecks(ctx context.Context, controlPlane *internal.ControlPlane, excludeFor ...*clusterv1.Machine) ctrl.Result {
	...
	if conditions.GetReason(controlPlane.KCP, controlplanev1.KubeadmControlPlaneCertificatesAvailableCondition) == controlplanev1.KubeadmControlPlaneCertificatesNotAvailableReason {
		controlPlane.PreflightCheckResults.CertificateMissing = true
		log.Info("Certificates are missing, can't join a new machine")
