...
func (w *Workload) UpdateStaticPodConditions(ctx context.Context, controlPlane *ControlPlane) {
	...
	for i := range controlPlane.Machines {
		...
		if !found {
			// tries to get the node by name, so we can detect when a node is missing or when a node exist but it does not have the CP label.
	...
	// Best effort check for the kubeadm default taint.
