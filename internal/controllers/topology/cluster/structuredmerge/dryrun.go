...
func dryRunSSAPatch(ctx context.Context, dryRunCtx *dryRunSSAPatchInput) (bool, bool, string, error) {
	...
	if hasChanges {
		...
		if len(diff.Object) != 0 {
			diffPatch, err := json.Marshal(diff.Object)
			if err != nil {
				return false, false, "", errors.Wrapf(err, "failed to marshal diff")
			}

			originalJSONWithChanges, err := jsonpatch.MergePatch(originalJSON, diffPatch)
			if err != nil {
			...
			changes = cmp.Diff(string(originalYAML), string(originalYAMLWithChanges))
