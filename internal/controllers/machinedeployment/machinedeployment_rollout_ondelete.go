...
func (p *rolloutPlanner) reconcileOldMachineSetsOnDelete(ctx context.Context) {
	...
	// Start scaling down old machine sets to acknowledge deleted replicas.
	// Note: deleted replicas must be considered no matter of scale down in MD;
	// in case both exist, deleted replicas should be deducted from the
	// totalScaleDownCount.
	for _, oldMS := range p.oldMSs {
	...
	// Scale down additional replicas if deleted replicas were not enough to align to MD replicas.
	for _, oldMS := range p.oldMSs {
