...
type rolloutOnDeleteSequenceTestCase struct {
	name string
...
func Test_rolloutOnDeleteSequences(t *testing.T) {
	ctx := context.Background()
	ctx = ctrl.LoggerInto(ctx, klog.Background())

	tests := []rolloutOnDeleteSequenceTestCase{
		{ // delete 1
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if testWithPredictableReconcileOrder {
				...
				t.Run("default", func(t *testing.T) {
					runRolloutOnDeleteTestCase(ctx, t, tt)
				})
			}
...
func runRolloutOnDeleteTestCase(ctx context.Context, t *testing.T, tt rolloutOnDeleteSequenceTestCase) {
	t.Helper()
	g := NewWithT(t)
	...
	for {
		taskList := getTaskListRolloutOnDelete(current)
...
func getTaskListRolloutOnDelete(current *rolloutScope) []string {
