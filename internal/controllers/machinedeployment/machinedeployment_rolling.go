...
func computeDesiredMS(ctx context.Context, deployment *clusterv1.MachineDeployment, currentMS *clusterv1.MachineSet) (*clusterv1.MachineSet, error) {
	...
	desiredMS := &clusterv1.MachineSet{
		...
		// NOTE: Carry over status from current MS, because it is used later on in the planning phase.
		// This will be dropped from the SSA intent by the SSA helper.
		// FIXME: it will be cleaner we can read from p.originalMS; the issue is that we are relying on md.util func that are not aware of p.originalMS
		Status: status,
	}
...
func (p *rolloutPlanner) reconcileNewMachineSet(ctx context.Context) error {
	...
	// remove the disable machine create annotation from new MachineSets that were created during reconcileOldMachineSetsOnDelete.
	// FIXME: check this, it should not be necessary because this annotation is not added by computeDesiredMS.
	// FIXME: test
...
func (p *rolloutPlanner) createOrUpdateMachineSets(ctx context.Context, c client.Client, recorder record.EventRecorder, ssaCache ssa.Cache) error {
	...
	for _, ms := range allMSs {
		...
		if ms.GetUID() == "" {
			...
			// FIXME: report back creation timestamp,
			continue
		}
