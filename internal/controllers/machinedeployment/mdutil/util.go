...
func NewMSNewReplicas(deployment *clusterv1.MachineDeployment, allMSs []*clusterv1.MachineSet, newMSReplicas int32) (int32, string, error) {
	...
	case clusterv1.RollingUpdateMachineDeploymentStrategyType:
		...
		if currentMachineCount >= maxTotalMachines {
			// Cannot scale up.
			return newMSReplicas, fmt.Sprintf("%d current Machines > %d MachineDeployment replicas + %d surge", currentMachineCount, ptr.Deref(deployment.Spec.Replicas, 0), maxSurge), nil
		}
		// Scale up.
		...
		return newMSReplicas + scaleUpCount, fmt.Sprintf("%d current Machines < %d MachineDeployment replicas + %d surge", currentMachineCount, ptr.Deref(deployment.Spec.Replicas, 0), maxSurge), nil
