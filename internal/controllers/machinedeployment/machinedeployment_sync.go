...
func (r *Reconciler) sync(ctx context.Context, md *clusterv1.MachineDeployment, msList []*clusterv1.MachineSet, templateExists bool) error {
	...
	//  use rollout planner
	//  init
	//  save changes so newMS is created
	// 	TBD if to rollback changes to OriginalMD
	//		it relies on:
	//			spec.Replicas
	//			DesiredReplicasAnnotation << check if set on newMS/oldMS
	// 			creation timestamp
	//  call scale
	//  TBD if to move scale into rollout planner as a follow up (!)
...
func (r *Reconciler) scaleMachineSet(ctx context.Context, ms *clusterv1.MachineSet, newScale int32, deployment *clusterv1.MachineDeployment) error {
	...
	// FIXME: drop annotation management
