...
func (r *Reconciler) sync(ctx context.Context, md *clusterv1.MachineDeployment, msList []*clusterv1.MachineSet, templateExists bool) error {
	// Use the rollout planned to take benefit of the common logic for:
	...
	// - applying changes to MS via SSA patches.
	planner := newRolloutPlanner()
	if err := planner.init(ctx, md, msList, nil, true, templateExists); err != nil {
		return err
	}

	// FIXME: When rollout is paused the code assumes it can always scale up oldMS (see scale in machinedeployment_sync.go).
	//   however old implementation is of missing removal of the DisableMachineCreateAnnotation that might exist or not
	...
	// Call the scale logic from the reconciler.
	// NOTE: this scale logic do not relies yet on the rollout planner, and it still relies on many
	// patch calls vs a single SSA call based on a carefully crafted desired state.
	// However, this is not an issue because the scale relies on info that are part of the desired state
	// and more specifically:
	// - metadata.CreationTimestamp
	// - metadata.Annotations (DesiredReplicasAnnotation, MaxReplicasAnnotation)
	// - spec.Replicas
	// - status.Replica counters
	...
	// TODO(in-place): once scale will be moved into rollout planner, consider if to move also syncDeploymentStatus +
	//  cleanupDeployment to the rollout planner (~same for RollingUpdate and OnDelete).
